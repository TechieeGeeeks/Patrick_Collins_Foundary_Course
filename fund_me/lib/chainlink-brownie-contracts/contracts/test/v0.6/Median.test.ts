  ...
  describe("testing various lists", () => {
    const tests = [
      ...
      },
      {
        ...
        want: BigNumber.from("57896044618658097711785492504343953926634992332820282019728792003956564819967"),
      },
      {
        name: "overflowing numbers",
        responses: [
          BigNumber.from("57896044618658097711785492504343953926634992332820282019728792003956564819967"),
          BigNumber.from("57896044618658097711785492504343953926634992332820282019728792003956564819966"),
        ],
  ...
  describe("validate sorting network", () => {
    ...
    xit("zero-one principle", async () => {
      ...
      for (let n = 0; n < (1 << 7) - 1; n++) {
        const list = [(n >> 6) & 1, (n >> 5) & 1, (n >> 4) & 1, (n >> 3) & 1, (n >> 2) & 1, (n >> 1) & 1, (n >> 0) & 1];
